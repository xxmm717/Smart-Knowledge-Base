...
> 文档版本：v1.2
> 更新时间：2026-07-29
> 当前设计边界：知识库导入流程，截止 `node_import_milvus`
...
8. 将 `item_name` 与 Chunk 正文拼接后分批生成 BGE-M3 稠密、稀疏向量，并写回内存中的 Chunk；
9. 自动创建并维护切片级集合 `zhiku.kb_chunks`，按 `item_name` 幂等清理、批量写入，并将 Milvus 主键回填到 Chunk。

当前阶段最准确的描述是：

> 文档预处理、切片、主体识别、文档级实体索引、Chunk 双向量化和 Chunk 级 Milvus 持久化已经分别跑通；完整七节点拓扑已经写出，但统一工作流入口尚未完成端到端收口。

因此，“七个节点均有主要业务实现”不等于“整个知识库导入模块已经完成”。目前系统已经形成文档级实体索引和可持久化的切片级向量集合，但正式入口的状态契约、异常结果和端到端验证仍需收口。
...
| Chunk 混合向量 | 已有主要实现 | `item_name + content`、每批 5 条、dense + sparse 写回 Chunk |
| Chunk 向量真实验证 | 已完成 | HAK180 的 6 个真实 Chunk 按 `5 + 1` 两批在 RTX 4070 上通过 |
| `kb_chunks` 持久化 | 已有主要实现并完成真实验证 | 自动建集合和索引、按 `item_name` 幂等替换、批量插入、主键回填 |
...
flowchart TD
    ...
    LLM["文本大模型"]
    BGE["本地 BGE-M3"]
    CHUNK_BGE["node_bge_embedding"]
    IMPORT["node_import_milvus"]
    JSON["本地 chunk.json"]
    DEBUG_JSON["chunks_with_embeddings_debug.json<br/>仅调试入口"]
    ENTITY["Milvus: zhiku.kb_item_names"]
    CHUNKS["Milvus: zhiku.kb_chunks"]
    ...
    ITEM <--> BGE
    ITEM --> ENTITY
    ITEM -->|带 item_name 的 Chunk| CHUNK_BGE
    CHUNK_BGE <--> BGE
    CHUNK_BGE -->|仅 main 调试流程| DEBUG_JSON
    CHUNK_BGE -->|内存中的带双向量 Chunk| IMPORT
    IMPORT --> CHUNKS
```
...
- `node_bge_embedding` 把 Chunk 双向量写回运行时状态，`main` 调试入口可额外生成完整向量 JSON。
- `node_import_milvus` 将运行时 Chunk 写入 `zhiku.kb_chunks`，并把 Milvus 生成的 `chunk_id` 回填到状态。

主体识别后写入 Chunk 的 `item_name` 和随后生成的双向量，标准工作流中都只存在于运行时状态；节点不会重新覆盖前一步的 `chunk.json`。`chunks_with_embeddings_debug.json` 是人工调试产物，不是正式节点契约。`node_import_milvus` 直接消费内存状态，而不能假设旧 JSON 已经包含 `item_name` 或向量。
...
需要注意：这是已经声明的图结构，不是已经稳定贯通的端到端能力。当前类式入口本身存在 `logger` 导入错误，PDF 输出目录字段也未统一；`node_bge_embedding` 与 `node_import_milvus` 均已实现主要逻辑，但尚未通过正式工作流入口完成端到端验收。
...
| `local_file_path` | 原始导入文件 | 外部调用方 | `node_entry` |
...
| `chunks` | 切片字典列表；向量化后每项增加 dense/sparse | `node_document_split`、`node_bge_embedding` | 主体识别、Chunk 向量化及入库节点 |
    ...
    "chunks": [
        {
            ...
            "parent_title": "# HAK 180 烫金机",
            "item_name": "BrotherHAK180烫金机",
            "dense_vector": [0.004349, 0.005890, -0.091614, "..."],
            "sparse_vector": {6: 0.18, 152: 0.41}
        }
    ]
}
```

这里的 Dense 是固定 1024 维普通 Python 列表，Sparse 是只保留非零维度及权重的字典。向量与输入 Chunk 依靠列表下标一一对应。
    ...
    "chunks": [
        {
            ...
            "dense_vector": [0.004349, 0.005890, -0.091614, "..."],
            "sparse_vector": {6: 0.18, 152: 0.41},
            "chunk_id": "467989507432544662"
        }
    ]
}
```

节点会先移除上游可能携带的 `chunk_id`，由 Milvus 的 `auto_id` 生成主键；插入成功后兼容读取 `ids` 或 `inserted_ids`，并按插入顺序将主键字符串回填到同一位置的 Chunk。该状态表示写入请求已成功完成，不替代后续检索可用性或工作流整体成功状态。
...
| 类型 | 字段/副作用 |
| --- | --- |
| 输入 | `pdf_path`、`local_dir`（解析结果输出目录） |
...
- PDF 解析节点与入口节点统一使用 `local_dir` 作为解析结果输出目录；
...
| 层级 | 调用位置 | 编码文本 | 结果去向 |
| --- | --- | --- | --- |
| 文档主体 | `node_item_name_recognition` | `item_name` | 直接写入 `kb_item_names` |
| 知识切片 | `node_bge_embedding` | `商品：{item_name}:介绍：{content}` | 写回 `chunks[*]` 的向量字段 |

这两次编码不是重复工作：主体向量用于判断“用户在问哪个商品”，Chunk 向量用于判断“该商品的哪一段内容可以回答问题”。

### 6.5 `node_bge_embedding` 当前实现

#### 输入输出契约

| 类型 | 字段/副作用 |
| --- | --- |
| 输入 | 非空 `chunks` 列表；每项实际需要 `item_name`、`content` |
| 输出 | 每个成功 Chunk 增加 `dense_vector`、`sparse_vector` |
| 模型副作用 | 加载或复用本地 BGE-M3，执行 GPU/CPU 编码 |
| 标准文件副作用 | 无 |
| 调试文件副作用 | `main` 入口生成 `chunks_with_embeddings_debug.json` |

#### 节点处理流程

```mermaid
flowchart LR
    A["校验 chunks 为非空列表"] --> B["获取 BGE-M3 单例"]
    B --> C["每批最多 5 个 Chunk"]
    C --> D["拼接 item_name 与 content"]
    D --> E["generate_embedding 批量编码"]
    E --> F["按批内下标绑定 dense / sparse"]
    F --> G["复制 Chunk 并汇总 output_data"]
    G --> H["写回 state.chunks"]
```
...
总数为 6 时，`range(0, 6, 5)` 产生起点 `0、5`，因此真实 HAK180 样例会按 `5 + 1` 两批执行。最后一批通过 Python 列表切片自动取得剩余数据。

成功批次中，节点对每个原始 Chunk 调用 `copy()`，再按照相同批内下标绑定：
...
这样既保持输入顺序，也避免在成功路径中直接给上游 Chunk 增加向量字段。
...
将商品名放在正文前面，可以为省略主语的局部段落补充商品上下文。但所有同商品 Chunk 都带相同前缀，可能提高同商品切片之间的共同相似度。该格式已经实现，不代表已经通过检索指标证明为最优，后续仍应与 `title + content`、仅 `content` 等方案做召回评估。

#### 当前降级语义

| 故障 | 当前结果 |
| --- | --- |
| `chunks` 为空或类型错误 | 抛出后被 `process()` 捕获，原状态返回 |
| 模型初始化失败 | 抛出后被 `process()` 捕获，原状态返回 |
| 单个批次缺少字段或编码失败 | 记录异常，该批原始 Chunk 无向量保留，继续下一批 |
| 模型返回空结果 | 该批原始 Chunk 无向量保留，继续下一批 |
| 成功 | 全部成功 Chunk 带双向量写回 `state["chunks"]` |

由于状态中仍没有 `degraded/errors`，调用方只能逐项检查向量字段或解析日志，才能发现“部分批次成功、部分批次降级”。另外，`step_2_init_model()` 返回的模型参数当前只承担提前初始化和校验作用；实际编码仍由 `generate_embedding()` 再次获取同一个缓存单例。

### 6.6 HAK180 调试入口

`node_bge_embedding.py` 提供 `run_hak180_debug()`，读取已有的 6 个 HAK180 Chunk，补充 `BrotherHAK180烫金机` 标签并执行完整节点：

```powershell
.\.venv\Scripts\python.exe -m processor.import_process.nodes.node_bge_embedding
```

终端只打印标题、Dense 维度、Sparse 非零项数量和 Dense 前 5 个值，避免输出整屏向量。完整结果写入：

```text
output/hak180产品安全手册/chunks/chunks_with_embeddings_debug.json
```

该 JSON 用于人工调试并位于被 Git 忽略的 `output/` 下，不应作为正式工作流的持久化契约。
...
### 7.4 已实现：`zhiku.kb_chunks`

`node_import_milvus` 消费 `state["chunks"]` 中已生成双向量的切片。集合不存在时，节点会创建下列显式 Schema，并同时创建稠密、稀疏向量索引：

| 字段 | Milvus 类型 | 约束/用途 |
| --- | --- | --- |
| `chunk_id` | `INT64` | 自增主键，`auto_id=True` |
| `content` | `VARCHAR(65535)` | 作为回答证据的正文 |
| `title` | `VARCHAR(65535)` | 当前章节或子块标题 |
| `parent_title` | `VARCHAR(65535)` | 原始章节关联 |
| `part` | `INT8` | 二次切分顺序 |
| `file_title` | `VARCHAR(65535)` | 来源文档过滤和追溯 |
| `item_name` | `VARCHAR(65535)` | 当前幂等键和主体过滤 |
| `dense_vector` | `FLOAT_VECTOR(1024)` | 语义检索 |
| `sparse_vector` | `SPARSE_FLOAT_VECTOR` | 关键词和型号检索 |

集合启用 `enable_dynamic_field=True`，因此保留了扩展切片元数据的空间。当前索引与 BGE-M3 的输出特点一致：

| 索引名 | 字段 | 类型 | 度量 | 参数 |
| --- | --- | --- | --- | --- |
| `dense_vector_index` | `dense_vector` | HNSW | COSINE | `M=16`、`efConstruction=200` |
| `sparse_vector_index` | `sparse_vector` | SPARSE_INVERTED_INDEX | IP | `DAAT_MAXSCORE`、`quantization=none` |

入库流程如下：

```text
校验非空 chunks 和 dense_vector
-> 连接 Milvus，按需建集合和双索引
-> 收集并去重 item_name
-> 删除同 item_name 的历史 Chunk，flush
-> 批量 insert
-> 读取 ids / inserted_ids
-> 按输入顺序回填 state.chunks[*].chunk_id
```

当前幂等键是 `item_name`，不区分来源文件；同一商品被不同文档识别时，后一次导入会删除前一次的 Chunk。删除和插入不是事务，新写入失败时同名旧数据已经被移除。后续检索模块尚未实现，但切片级数据契约和持久化能力已具备。
...
| MinIO | `minio_config` |
| 切片和上下文参数 | 多数为节点内常量 |
| Chunk 向量批大小 | `node_bge_embedding` 内固定为 5 |
| Chunk 编码模板 | `node_bge_embedding` 内固定拼接字符串 |
...
| 批次降级 | `node_bge_embedding` | 失败批次保留无向量原数据，继续处理后续批次 |
...
- 识别成功但向量失败；
- 识别成功但入库失败；
- 部分 Chunk 有向量、部分 Chunk 因批次失败而没有向量；
- 使用文件名兜底；
- 节点发生异常但已被吞掉。
...
### 11.2 HAK180 真实 BGE-M3 测试

新增测试：

```text
tests/nodes/test_node_bge_embedding_integration.py
```

该测试不 Mock `generate_embedding`，而是读取 `output/hak180产品安全手册/chunks/chunks.json` 的 6 个真实 Chunk，补充 `BrotherHAK180烫金机` 后调用完整节点。验证内容包括：

- 6 个 Chunk 按 `5 + 1` 两批全部完成；
- 输出数量、顺序和原业务字段保持不变；
- 每条 Dense 都是 1024 维有限数值且 L2 模长约为 1；
- 每条 Sparse 都是非空字典，索引和权重类型合法；
- 不同正文产生不同 Dense；
- 成功路径不会直接给测试保留的原始 Chunk 副本增加向量字段。
...
随后与 PDF、主体识别快速测试共同执行：

```text
32 passed in 31.74s
```

这里使用的是本地真实 BGE-M3 模型，不调用远程 HTTP API，也不写入 Milvus。

### 11.3 HAK180 真实 Milvus 入库测试

新增测试：

```text
tests/nodes/test_node_import_milvus_integration.py
```

该测试连接真实 `zhiku.kb_chunks`，读取 `chunks_with_embeddings_debug.json` 中的 6 个 HAK180 双向量 Chunk，并将 JSON 反序列化后的 Sparse 字典键恢复为整数。测试为本次运行生成唯一的 `item_name`，避免与手工保留数据混淆；插入后查询验证，再在 `finally` 中清理该测试标签的数据。

验证内容包括：

- 集合存在，且 `dense_vector_index`、`sparse_vector_index` 均可见；
- 6 条数据全部插入，6 个回填 `chunk_id` 均为唯一数字；
- flush/load 后可按唯一 `item_name` 查询到 6 条数据；
- 查询到的 `title` 集合与输入 Chunk 一致；
- 测试清理只作用于本次随机标签，不影响手工保留的 HAK180 数据。
...
此外，按完整节点链路直接执行 `chunks.json -> NodeBgeEmbedding -> NodeImportMilvus`，已将 `BrotherHAK180烫金机` 的 6 个 Chunk 持久化到 `zhiku.kb_chunks`；该批数据保留在真实 Milvus 中，供 Attu 和后续检索调试使用。

2026-07-29 再次联合执行 BGE-M3 与 Milvus 两项真实集成测试，结果为：
...
├── images/                 # 8 个本地图片文件
└── chunks/
    ├── chunks.json                         # 6 个 Chunk，旧命名格式
    └── chunks_with_embeddings_debug.json   # main 调试入口生成，非正式产物
```
...
| 真实 Chunk 向量批次 | `5 + 1` |
| 每条 Dense 维度 | 1024 |
| 每条 Sparse 非零项 | 149～212 |
| 成功生成双向量的 Chunk | 6/6 |

HAK180 的 `chunks.json` 是较早命名的保留产物，最大 Chunk 长度为 2142，和当前代码的 `chunk.json` 文件名及 2000 字符目标存在版本差异。它适合作为实体识别和向量化的真实输入样例，不应单独用来证明当前切片代码严格满足最新约束。`chunks_with_embeddings_debug.json` 则用于人工查看完整向量，不替代 `kb_chunks` 持久化。
...
- 当前总行数为 1；
- HAK180 数据可查询；
- `kb_chunks` 已存在，Dense、Sparse 两个索引均已建立；
- `BrotherHAK180烫金机` 当前保留 6 条 Chunk 级真实数据，可按 `item_name` 查询。
...
2. 当前测试以节点直调或手工串接为主，没有覆盖正式工作流入口。
...
chunks[*] -> dense/sparse -> state.chunks[*]
state.chunks[*] -> kb_chunks -> 回填 chunk_id
```

因此 Chunk 可以作为 Milvus 中的具体手册证据被查询；但查询编排、Dense/Sparse 混合召回、重排和回答生成尚未实现。
...
| Chunk 向量批大小 | 未统一声明 | 节点常量 5 |
| Chunk 编码模板 | 未统一声明 | `商品：item_name:介绍：content` |
...
当前各节点的异常处理侧重点不同。下一阶段实现 Chunk 批量入库前，应先定义：
...
2. 保持 `local_dir` 作为唯一的解析结果输出目录字段；
3. 保证直接导入无图片 Markdown 时也把文件正文写回 `md_content`；
4. 明确 `chunk.json` 是中间快照还是最终可重放数据；
5. 增加从 `node_entry` 到 `node_import_milvus` 的工作流测试；
6. 在状态中记录 `degraded/failed` 等执行结果。

完成这些工作后，七个节点才能从分别可用提升为稳定的导入子流程。

### 13.2 稳定 `node_bge_embedding`

当前节点已经确定的主要行为：

| 类型 | 当前字段/策略 |
| --- | --- |
| 必需输入 | `chunks[*].item_name/content` |
| 透传元数据 | `title/parent_title/file_title/part` 等原 Chunk 字段 |
| 实际编码 | `商品：item_name:介绍：content` |
| 批大小 | 固定 5 |
| 输出 | `chunks[*].dense_vector/sparse_vector` |
| 失败策略 | 按批降级，失败批次保留原始 Chunk |

后续仍需收口：

- 让批大小从统一配置读取，并按显存和文档规模评估；
- 为每个 Chunk 增加明确的向量化成功/失败状态；
- 校验模型输出数量与批次输入数量严格一致；
- 消除提前初始化参数与 `generate_embedding()` 内部单例获取之间的冗余；
- 模型名称、模型版本和归一化配置是否作为元数据保存。

当前 `item_name + content` 的价值是补足切片省略的主语；但如果每个 Chunk 都加入相同商品名，也可能让同一商品下的切片过度相似。应通过检索评估与 `title + content`、仅 `content` 等方案对比后，再固定最终编码模板。

### 13.3 完善 `node_import_milvus`

当前节点已实现集合自动创建、同 `item_name` 清理、批量插入和主键回填。后续应保持职责单一，并完善以下边界：
...
- 是否将 `chunk_id` 保持为 Milvus 自增主键，还是额外引入业务稳定 ID；
- 文档重导入时按 `item_name + file_title`、文件哈希还是批次号删除；
- 已存在集合与新模型输出维度不一致时如何阻止错误写入；
- 批量写入大小如何配置；
- 删除旧数据和插入新数据如何避免中间空窗；
- 如何把写入数量、回填 ID 和异常写入结构化状态。
...
6. **按固定批次为 Chunk 生成双向量**，兼顾 GPU 显存占用和输入输出顺序；
7. **用 `kb_item_names` 和 `kb_chunks` 规划两级索引**，将“识别在问谁”和“找到具体证据”分开处理。

目前文档主体级索引和 Chunk 级持久化均已闭环，HAK180 的 6 个真实 Chunk 已经保留在 `zhiku.kb_chunks` 中。下一阶段应统一工作流和状态契约，并实现基于两级集合的混合检索、重排和回答生成。这样才能把已有的“带向量结构化知识切片”真正转变为可用的 RAG 问答能力。
...
| Chunk 向量节点 | `processor/import_process/nodes/node_bge_embedding.py` |
| Chunk 入库节点 | `processor/import_process/nodes/node_import_milvus.py` |
...
| Chunk 真实向量测试 | `tests/nodes/test_node_bge_embedding_integration.py` |
| Chunk 真实入库测试 | `tests/nodes/test_node_import_milvus_integration.py` |

## 附录 B：后续文档版本

| 版本 | 补充范围 | 状态 |
| --- | --- | --- |
| v1.0 | 截止 `node_item_name_recognition` | 历史版本 |
| v1.1 | `node_bge_embedding`、Chunk 向量契约与真实模型验证 | 历史版本 |
| v1.2 | `node_import_milvus`、`kb_chunks` 和真实 Milvus 入库验证 | 当前版本 |
| v2.0 | 七节点统一工作流收口 | 待补充 |
| v3.0 | 混合检索、重排和回答生成 | 待补充 |
