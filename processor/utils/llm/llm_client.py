...
def get_llm_client(model: Optional[str] = None, json_mode: bool = False) -> ChatOpenAI:
    ...
    :param model: 模型名称，优先级：传入参数 > 配置文件llm_config.llm_model > 内置默认qwen3-32b
    ...
    try:
        llm_client = ChatOpenAI(
            ...
            temperature=llm_config.llm_temperature or 0.1,  # 低温度保证输出确定性（0~1）
            api_key=llm_config.api_key,  # API密钥
            base_url=llm_config.base_url,  # API基础地址（适配国产模型代理地址）
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info("===== LLM客户端工具测试结束 =====")# 环境配置与依赖导入
import os
from dotenv import load_dotenv
from langchain_openai import ChatOpenAI
from langchain_core.exceptions import LangChainException
