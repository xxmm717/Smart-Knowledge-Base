...
def step_2_prepare_collection(vector_dimension: str):
    """
    ...
    if client is None:
        ...
        raise ValueError("Milvus 连接失败：get_milvus_client() 返回")
...
def step_4_insert_data(client, chunks_json_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    # PyMilvus 不同版本可能返回 inserted_ids 或 ids，优先兼容旧字段。
    inserted_ids = insert_result.get("inserted_ids") or insert_result.get("ids", [])
...
def create_collection(client, collection_name, vector_dimension):
    ...
    # 3.创建索引参数：为向量字段创建索引，提升检索性能
    index_params = client.prepare_index_params()

    # 优化稠密向量索引 HNSW + COSINE（恢复最佳性能配置）
    ...
    index_params.add_index(
        ...
        params={"inverted_index_alog": "DAAT_MAXSCORE","quantization":"none"}
    )
