...
class NodeMdImg(BaseNode):
    ...
    def process(self,state:ImportGraphState)->ImportGraphState:
        ...
        if not images_dir.exists():
            self.logger.info(f"图片文件夹不存在，跳过图片处理：{images_dir.absolute()}")
            return state

        # 2.扫描并筛选MD中引用的图片
        target_images = self._step_2_get_scan_images(md_content,images_dir)
        if not target_images:
            self.logger.info("未检测到MD中引用的支持格式图片，跳过后续处理")

        # 3.调用大模型生成图片摘要
        summaries = self._step_3_generate_summaries(md_path_obj.stem,target_images)

        # 4.上传图片至MinIO，替换md图片路径并填充摘要
        new_md_content = self._step_4_upload_and_replace(minio_client,md_path_obj.stem,target_images,summaries,md_content)
    ...
    def _step_1_get_content(self, state:ImportGraphState)->Tuple[str,Path,Path]:
        ...
        if not state["md_content"]:
            with open(md_path_obj,"r",encoding="utf-8") as f:
                md_content = f.read()
            self.logger.info(f"从文件读取md内容完成：文件大小：{len(md_content)}")
        else:
            md_content = state["md_content"]
            self.logger.info(f"从全局状态获取MD内容完成，内容大小：{len(md_content)} 字符")
    ...
    def _step_2_get_scan_images(self, md_content:str, images_dir:Path)-> list[Tuple[str,str,Tuple[str,str]]]:
        """
        ...
        for image_file in os.listdir(images_dir):
            ...
            # 过滤MD中未引用的图片
            if not context:
                self.logger.info(f"图片未在MD中引用，跳过处理：{image_file}")
                continue

            # 4.组装处理图片元数据，取第一个匹配的图片上下文
            target_images.append((image_file,image_path,context))
            self.logger.info(f"图片加入待处理列表：{image_file}")

        self.logger.info(f"图片扫描完成，共筛选出待处理图片：{len(target_images)} 张")
    ...
    def _find_image_in_md(self, md_content: str, image_file: str,context_len: int = 100)->Tuple[str,str]:
        ...
        # 打印图片上下文，便于调试
        self.logger.debug(f"图片[{image_file}]匹配到引用，上文：{pre_text.strip()}")
        self.logger.debug(f"图片[{image_file}]匹配到引用，下文：{post_text.strip()}")
    ...
    def _clean_minio_directory(self, minio_client: Minio, upload_dir_prefix)->None:
        """
    ...
    def _upload_images_batch(self, minio_client:Minio, upload_dir:str, targets: List[Tuple[str, str, Tuple[str, str]]])->Dict[str, str]:
        """
